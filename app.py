...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'e0453d9a196ca0784668571422242109'
...
def validate_player_field(field: StringField) -> bool:
    if field.data.strip() == '':
         return False
    return db.is_player_registered(field.data)

@app.route("/")
@app.route("/home")
def home():
    return render_template('index.html', number_of_players=db.get_number_of_players(), number_of_matches_played=db.get_number_of_matches(), 
    ...
    active_players_percentage=db.get_active_players_percentage(), age_gap=db.get_age_gap(), total_points=db.get_total_points(), total_coins=db.get_total_coins_played())
...
@app.route("/players")
def players():
    return render_template('players.html', title="Player List", players=db.get_all_the_players(), number_of_players=db.get_number_of_players())
...
def playerLogin():
    ...
    if form.validate_on_submit():
        if db.is_player_registered(form.player_name.data):
            player = db.get_player_by_nickname(form.player_name.data)
...
def addPlayer():
    ...
    if form.validate_on_submit(): 
        try:
            player = Player(form.first_name.data, form.last_name.data, form.date_of_birth.data, form.gender.data)
...
def removePlayer():
    ...
    if form.validate_on_submit():
        if db.is_player_registered(form.nickname.data):
            if db.validate_player_date_of_birth(form.nickname.data, form.date_of_birth.data):
                try: 
                    db.remove_all_the_player_results_by_nickname(form.nickname.data)           
                    db.remove_player_by_nickname(form.nickname.data) 
...
def renamePlayer():
    ...
    if form.validate_on_submit():
        if db.is_player_registered(form.current_nickname.data):
            if db.validate_player_date_of_birth(form.current_nickname.data, form.date_of_birth.data):
                try: 
                    db.change_player_nickname(form.current_nickname.data, form.new_nickname.data)
    ...
    if form.validate_on_submit(): 
        ...
        try: 
            ...
            for index, nickname in enumerate(player_nicknames):
                results.append(Result(date, index+1, db.get_player_by_nickname(nickname)))
